"""
Unit tests for SAC Agent implementation.
"""
...
from gfmf.reinforcement_learning.agents import SACAgent
...
class TestSACAgent(unittest.TestCase):
    ...
    def test_learn(self):
        ...
        # Note: For SAC with entropy tuning, the weights might not change much in a single step
        # We'll check that there's at least some small difference
        self.assertFalse(torch.allclose(initial_actor_weights, updated_actor_weights, atol=1e-6))
    ...
    def test_save_load_policy(self):
        ...
        # Save the policy
        model_path = os.path.join(self.temp_dir, "sac_test_model.pth")
        ...
        # Create a new agent
        new_agent = SACAgent(self.state_dim, self.action_dim)
